...
<section class="rocksite-s-content -margin-top">
    <div class="container rocksite-s-content__content">
        <div class="rocksite-t-content-layout -left-sidebar">
    <?php while ( have_posts() ) {
        ...
        <div class="wedocs-single-wrap">
            ...
            <div class="wedocs-single-content rocksite-t-content-layout__content">
                <?php wedocs_breadcrumbs(); ?>

                <article id="post-<?php the_ID(); ?>" <?php post_class(); ?> itemscope itemtype="http://schema.org/Article">
                    <div class="rocksite-o-content">
                    <header class="rocksite-o-content__header entry-header">
                        <?php the_title( '<h1 class="rocksite-o-content__header__title entry-title" itemprop="headline">', '</h1>' ); ?>

                        <?php if ( wedocs_get_option( 'print', 'wedocs_settings', 'on' ) == 'on' ) { ?>
                            <a href="#" class="wedocs-print-article wedocs-hide-print wedocs-hide-mobile" title="<?php echo esc_attr( __( 'Print this article', 'genui' ) ); ?>"><i class="wedocs-icon wedocs-icon-print"></i></a>
                        <?php } ?>
                    </header><!-- .entry-header -->

                    <div class="entry-content rocksite-o-content__entry" itemprop="articleBody">
                        <?php
                            the_content( sprintf(
                                /* translators: %s: Name of current post. */
                                wp_kses( __( 'Continue reading %s <span class="meta-nav">&rarr;</span>', 'genui' ), [ 'span' => [ 'class' => [] ] ] ),
                                the_title( '<span class="screen-reader-text">"', '"</span>', false )
                            ) );

                            wp_link_pages( [
                                'before' => '<div class="page-links">' . esc_html__( 'Docs:', 'genui' ),
                                'after'  => '</div>',
                            ] );
                            ...
                            if ( $tags_list ) {
                                printf( '<span class="tags-links"><span class="screen-reader-text">%1$s </span>%2$s</span>',
                                    _x( 'Tags', 'Used before tag names.', 'genui' ),
                                    $tags_list
                                );
                            }
                        ?>
                    ...
                    <footer class="entry-footer wedocs-entry-footer">
                        <?php if ( wedocs_get_option( 'email', 'wedocs_settings', 'on' ) == 'on' ) { ?>
                            <span class="wedocs-help-link wedocs-hide-print wedocs-hide-mobile">
                                ...
                                <?php printf( '%s <a id="wedocs-stuck-modal" href="%s">%s</a>', __( 'Still stuck?', 'genui' ), '#', __( 'How can we help?', 'genui' ) ); ?>
                            </span>
                        <?php } ?>
                        ...
                        <time itemprop="dateModified" datetime="<?php echo esc_attr( get_the_modified_date( 'c' ) ); ?>"><?php printf( __( 'Updated on %s', 'genui' ), get_the_modified_date() ); ?></time>
                    </footer>
